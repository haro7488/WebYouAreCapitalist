...
- [x] Result 화면에 순위 변동 표시
- [x] 경쟁사 행동 알림

### 엔진 리워크 ✅
- [x] types.ts 리워크: AP 삭제, 7섹터, 정부/인플레/목표 타입 추가
- [x] 데이터 JSON 분리 (assets, balance, names, traits, glossary, events — 6개)
- [x] 이벤트 시스템 리워크: 매 턴 고정 발생 + 특성 부여/제거 + 선언형 JSON 조건
- [x] 인플레이션 시스템 구현
- [x] 정부 이벤트 시스템 구현
- [x] 특성 엔진: 경제 계산에 선언형 효과 반영
- [x] 정보 기업 + 조사 시스템 리워크: 정보 자산 기반 무료 조사
- [x] 목표 시스템: 3개 중 1 선택, 보너스 순자산 가산
- [x] 파산 리워크: 마이너스 허용 + 이자 페널티
- [x] 이벤트 확장: 신규 8개 + 특성 전용 이벤트
- [x] 밸런스 조정 (시뮬레이션 기반 수치 튜닝)

### UI/UX 개선 ✅
- [x] 상단바 리워크 + 특성(Trait) UI
- [x] 현황 탭: 시장 상세 + 수익/지출 분리 + 섹터 트렌드
- [x] 정부 탭 + 인플레이션 표시
- [x] 7섹터 + 목표 선택 + 조사 횟수 + 파산 표시
- [x] 포트폴리오 업그레이드 비용/효과 표시
- [x] 현황판: 예상 수익 + 순자산 막대그래프 + 순위 변동 선그래프
- [x] 자산 마켓 섹터별 그룹핑
- [x] 이벤트 선택지 효과 미리보기
- [x] ActionBar: 탭 하이라이트 + 다음턴 분리
- [x] 조사 탭: 모달→인라인 뷰 전환 + 조사 기록 표시
- [x] 테스트 파라미터 패널 (DevPanel)
- [x] 게임 도움말 모달 + 용어 백과사전

### 글로서리 시스템 ✅
- [x] glossary.json 데이터 분리
- [x] Paradox 스타일 스택형 체이닝 팝오버
- [x] 게임 UI 전반 자동 감지 툴팁 적용 (전 컴포넌트)
- [x] aliases 대폭 보강 + 섹터별/UI 용어 신규 항목

### 문서 전면 개정 ✅
- [x] GDD + design docs 5종 Phase 2 설계 반영
...
| 2026-02-23 | Result 순위 UI + 경쟁사 알림 + DevPanel + 상단바/현황탭 리워크 |
| 2026-02-24 | 엔진 리워크 (7섹터/AP삭제/정부/인플레/특성/목표/파산) + 데이터 JSON 분리 + 글로서리 시스템 + 문서 전면 개정 |
| 2026-02-25 | ActionBar 탭 개선 + 조사 탭 인라인 뷰 전환 + 조사 기록 표시 |
